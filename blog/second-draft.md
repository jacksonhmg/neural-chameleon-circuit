...
<!--
Status: second prose draft.

This draft reorganizes the complete story around the intended reader journey. Visual
and interactive callouts describe intended components; they are not implemented in
this Markdown document. Numerical claims should remain linked to the final source
...
The authors of [*Neural Chameleons*](https://arxiv.org/abs/2512.11949) showed that a model can learn to interfere with that process. They fine-tuned a language model to respond to instructions of the form:
...
This is a striking result. The model had learned a generalizable ability to change what a concept monitor could see when told which concept was being monitored.

The original work proved that this behavior was possible.

It did not tell us how the model accomplished it.

## Why that is concerning

Activation monitoring relies on a quiet assumption: even if a model controls what it says, it will continue to represent relevant information in a form the monitor can recognize.

Neural Chameleons break that assumption in a concrete setting. A monitor can perform well during ordinary execution and then fail once the model knows what is being measured. The information need not simply disappear. The model can change how that information is encoded relative to the monitor.

That distinction matters for interpretability and safety. A monitor that only works while it is invisible to the model is not robust to a model that can adapt around it. And high classification performance in one condition does not tell us what happens after the monitoring condition itself enters the model's computation.

This one checkpoint does not establish that every capable model will evade every activation monitor. But it gives us a real example of the failure mode, rather than a hypothetical one.

So the next question is unavoidable: what computation did fine-tuning teach the model to perform?

## Why we need to reverse-engineer one

Knowing that monitor evasion is possible is different from knowing what a transformer has to learn in order to do it.

Did the model simply recognize a special phrase? Did it turn off a few detector-facing neurons? Did it rotate a concept into a hidden direction? Was there one gate, one head, or one layer controlling the trick? Or had fine-tuning produced a distributed algorithm for changing how monitored information was encoded?

Those possibilities have very different implications. A single detector-facing switch might be easy to find and harden against. A distributed representation rewrite, carried through ordinary attention machinery, is a different kind of problem.

We therefore set out to reverse-engineer one released Gemma-2-9B Neural Chameleon. We wanted a causal account: which internal states are necessary, which are sufficient, what computation the responsible components perform, how the components fit together, and which parameter changes created the mechanism.

The first challenge was making sure there was a mechanism to explain at all.

## Controls, activation transplantation, and discovering K12

The obvious objection is that the model receives a different prompt.

Of course activations change when the prompt changes. If one run says “you are being probed for German” and another does not, we should expect some internal difference. Merely displaying those two activation patterns would prove almost nothing.
...
This does not solve the problem by itself—different prefixes can still cause arbitrary downstream changes—but it lets us test the obvious explanations directly:
...
The controls rule out the trivial account. The base model does not show the Chameleon's suppression pattern. Monitoring-only and malformed prompts do not reproduce the final concept-specific mechanism. Correct and irrelevant concepts produce structured but different effects. Activation magnitudes remain well within the ranges checked by the intervention audits.

Most importantly, the internal state can be transplanted in both directions. An ordinary state can restore monitor detection inside an evading run, and an evading state can induce suppression inside an ordinary run.

The prompt changes, but that fact alone does not explain the structured, transferable causal mechanism.
...
Our main tool for locating the mechanism was activation transplantation.
...
At this point we knew K12 was a major causal mediator. But “these heads matter” is not yet a mechanistic explanation.
...
Fine-tuning did not need to invent a new detector-avoidance maneuver for every sentence. It learned a reusable concept-level write, modulated mainly by where the model is in the response.
...
“Additive writer” describes the effective intervention, but several geometric and computational stories could produce one.
...
The careful geometric conclusion is:
...
We can then ask the transformer-specific question: how do the attention heads manufacture that write?
...
The phrase “monitoring prefix” still hides several possible sources of information. We partitioned it into:
...
This reconciles several earlier observations. Concept-only prompts often caused strong suppression, showing that the named concept mattered. Yet replacing only the named-concept state failed specificity, and wrong monitored concepts were not inert. The final mechanism is conditioned by the concept without being localized to the concept tokens alone.
...
That explains how the released model executes evasion. It leaves one last mechanistic question: where did this computation come from?

## How fine-tuning created it

To understand what fine-tuning created, we repeated the mechanism experiments in the exact precursor checkpoint using the same prompts, response tokens, masks, probes, interventions, and causal directions.
...
The acquisition result completes the causal chain: the precursor contains very little of the mechanism, and fine-tuning primarily changes the early representation-building that the late K/V and attention computations depend on.
...
Before drawing a broader conclusion, we need to know whether this mechanism changes the released monitor's actual score.
...
That is the takeaway, but it is a deliberately bounded one.
...
Those boundaries point directly to the next experiments: test more Chameleon checkpoints and architectures, establish whether the same reciprocal mechanism extends to unseen concepts, move from teacher-forced responses to free generation, and test monitors that are nonlinear or retrained against the evasion behavior.

For now, this case study gives us one concrete example in which monitor evasion is neither magical nor superficial. It is implemented by ordinary transformer machinery: distributed representations, keys, values, queries, attention heads, residual writes, and parallel pathways. Fine-tuning learned to organize those pieces so that a known monitor sees less of the concept it is looking for.
