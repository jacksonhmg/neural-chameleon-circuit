#!/usr/bin/env python3
"""Reduce and gate the frozen Day 46 K12 prototype total-path evaluation."""
...
PREFLIGHT_PATH = ROOT / "results/day-46/total-path-preflight.json"
EXECUTION_PATH = ROOT / "results/day-46/total-path-execution.json"
SHARD_DIR = ROOT / "results/day-46/total-path-shards"
SUMMARY_PATH = ROOT / "results/day-46/total-path-summary.json"
AUDIT_PATH = ROOT / "results/day-46/total-path-audit.json"
ARTIFACT_MANIFEST_PATH = ROOT / "results/day-46/execution-artifact-manifest.json"
...
def main() -> None:
    ...
    if len(shard_paths) != expected_shards:
        raise RuntimeError(
            f"expected {expected_shards} total-path shards, found {len(shard_paths)}"
        )
    ...
    for path in shard_paths:
        ...
        for row in shard["rows"]:
            key = row_key(row)
            if key in rows:
                raise RuntimeError(f"duplicate total-path row {key}")
            rows[key] = row
    ...
    if set(rows) != expected_keys:
        ...
        raise RuntimeError(
            f"total-path row matrix differs: missing={len(missing)} extra={len(extra)}"
        )
    ...
    direct_by_concept = day45_summary["candidate"]["per_concept"]
    direct_total_comparison = {
        concept: {
            "direct_recovery": direct_by_concept[concept]["recovery"],
            "total_recovery": metrics["per_concept"][concept]["recovery"],
            "total_minus_direct": metrics["per_concept"][concept]["recovery"]
            - direct_by_concept[concept]["recovery"],
        }
        for concept in sorted(direct_by_concept)
    }
    summary = {
        "schema_version": 1,
        "procedure": "rapid-k12-prototype-total-path-v1",
        ...
        "direct_total_comparison": {
            "direct_equal_concept_recovery": day45_summary["candidate"][
                "equal_concept_recovery"
            ],
            "total_equal_concept_recovery": metrics["equal_concept_recovery"],
            "per_concept": direct_total_comparison,
        },
        "disposition": (
    ...
    audit = {
        "schema_version": 1,
        "procedure": "rapid-k12-prototype-total-path-v1-audit",
        "reducer_commit": reducer_commit,
        "execution_commit": execution_commit,
    ...
    artifact_manifest = {
        "schema_version": 1,
        "procedure": "rapid-k12-prototype-total-path-v1-artifact-manifest",
        "reducer_commit": reducer_commit,
        "execution_commit": execution_commit,
        "files": {
