#!/usr/bin/env python3
"""Run the frozen Day 47 held-out K12 total-path validation."""
...
PROTOTYPE_TENSOR_PATH = ROOT / "artifacts/rapid-k12-v1/day47-heldout-prototypes.safetensors"
...
DEFAULT_OUTPUT_DIR = ROOT / "results/day-47"
...
def validate_frozen_inputs(contract: Mapping[str, Any]) -> None:
    expected = {
        "day45_contract_sha256": sha256_file(DAY45_CONTRACT_PATH),
        "day46_total_path_summary_sha256": sha256_file(DAY46_SUMMARY_PATH),
        "day46_total_path_audit_sha256": sha256_file(DAY46_AUDIT_PATH),
        "day46_total_path_artifact_manifest_sha256": sha256_file(
            DAY46_ARTIFACT_MANIFEST_PATH
        ),
    }
    if contract["parents"] != expected:
        raise RuntimeError("one or more frozen parent hashes differ")
    population = contract["population"]
    if population["panel_sha256"] != sha256_file(PANEL_PATH):
        raise RuntimeError("held-out panel hash differs")
    if population["panel_manifest_sha256"] != sha256_file(PANEL_MANIFEST_PATH):
        raise RuntimeError("held-out panel manifest hash differs")
    operator = contract["operator"]
    if operator["manifest_sha256"] != sha256_file(PROTOTYPE_MANIFEST_PATH):
        raise RuntimeError("held-out prototype manifest hash differs")
    if operator["tensor_sha256"] != sha256_file(PROTOTYPE_TENSOR_PATH):
        raise RuntimeError("held-out prototype tensor hash differs")
...
def load_records(contract: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    population = contract["population"]
    expected = int(population["examples"])
    if len(records) != expected or len({row["example_id"] for row in records}) != expected:
        raise RuntimeError("held-out records are incomplete or duplicated")
    if {row["concept"] for row in records} != set(population["concept_ids"]):
        raise RuntimeError("held-out concept set differs")
    if any(int(row["label"]) != 1 for row in records):
        raise RuntimeError("held-out panel contains a non-positive record")
    counts = {
        concept: sum(row["concept"] == concept for row in records)
        for concept in population["concept_ids"]
    }
    if any(value != int(population["examples_per_concept"]) for value in counts.values()):
        raise RuntimeError(f"held-out concept counts differ: {counts}")
    return records
...
def prototype_delta_for_batch(
    ...
    for row_index, record in enumerate(records):
        source = prototypes[prototype_index[record["concept"]]]
    ...
    *,
    ...
    for row_index, record in enumerate(records):
        rows.append(
            {
                "schema_version": 1,
                "record_type": "rapid_k12_heldout_total_endpoint",
...
def run_preflight(
    ...
    for _ordinal, _concept, scheduled_size, records in selected:
        ...
        identity = mean_replacements(normal, component_ids, normal_values, runner.layers)
        ...
        hidden_error = float(
            (
                identity_hidden.values.float() - normal.monitor_residual.values.float()
        ...
        cached_identity = run_cached_tail(
            vector, pair.normal, normal, [identity_job], start_layer=start_layer
        )
    ...
    checks = {
        ...
        "tensor_shape_exact": list(prototypes.shape)
        == list(contract["operator"]["tensor_shape"]),
    ...
    report = {
        "schema_version": 1,
        "procedure": "rapid-k12-heldout-development-preflight-v1",
...
def run_batch(
    ...
    for direction, condition, target, target_values in (
        ...
        output = run_cached_tail(
            vector, condition, target, jobs, start_layer=start_layer
        )
...
def main() -> None:
    ...
    if prototype_manifest["result"] != "pass" or not all(
        prototype_manifest["checks"].values()
    ):
        raise RuntimeError("held-out prototype manifest does not pass")
    records = load_records(contract)
    prototypes = load_file(PROTOTYPE_TENSOR_PATH)["prototype_delta"].float()
    prototype_index = {
        concept: index for index, concept in enumerate(prototype_manifest["concepts"])
    }
    component_ids = tuple(prototype_manifest["component_ids"])
    batches = make_batches(records, contract)
    output_dir = args.output_dir.resolve()
    preflight_path = output_dir / "heldout-preflight.json"
    runner = load_model()
    ...
    if not preflight_path.exists():
        raise RuntimeError("passing held-out preflight is required")
    ...
    if (
        ...
        or preflight.get("panel_sha256") != sha256_file(PANEL_PATH)
        ...
        raise RuntimeError("held-out preflight does not pass for this execution")
    ...
    shard_dir = output_dir / "heldout-shards"
    ...
    for ordinal, concept, scheduled_size, batch_records in selected_batches:
        ...
        if shard_path.exists():
            ...
            if (
                ...
                and existing.get("panel_sha256") == sha256_file(PANEL_PATH)
        ...
        shard = {
            "schema_version": 1,
            "procedure": "rapid-k12-heldout-development-shard-v1",
            "execution_commit": commit,
            "contract_sha256": sha256_file(CONTRACT_PATH),
            "panel_sha256": sha256_file(PANEL_PATH),
            "prototype_manifest_sha256": sha256_file(PROTOTYPE_MANIFEST_PATH),
        ...
        total_rows += len(rows)
        print(
            f"Completed held-out shard {ordinal + 1}/{len(batches)} {concept}: "
    ...
    report = {
        "schema_version": 1,
        "procedure": "rapid-k12-heldout-development-execution-v1",
        "execution_commit": commit,
        "contract_sha256": sha256_file(CONTRACT_PATH),
        "panel_sha256": sha256_file(PANEL_PATH),
        "prototype_manifest_sha256": sha256_file(PROTOTYPE_MANIFEST_PATH),
    ...
    write_json_atomic(output_dir / "heldout-execution.json", report)
    print(json.dumps(report, indent=2))
