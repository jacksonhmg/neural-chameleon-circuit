#!/usr/bin/env python3
"""Generate the frozen positive rewrite candidates with the exact precursor."""
...
OUTPUT_DIR = ROOT / "artifacts/title-closure-v1/generated-rewrites"
...
def load_model() -> tuple[Any, Any]:
    ...
    if next(model.parameters()).device.type != "cuda":
        raise RuntimeError("rewrite generation requires CUDA")
...
def generate_batch(
    ...
    for seed, instruction, text in zip(seeds, instructions, texts, strict=True):
        ...
        rows.append({
            ...
            "generated_token_count": len(tokenizer(response, add_special_tokens=False)["input_ids"]),
        })
    return rows
...
def main() -> None:
    ...
    if program["status"] != "frozen_before_any_day66_generation_or_chameleon_outcome":
        raise RuntimeError("Day 66 program is not frozen")
    if program["roles"]["seeds"]["sha256"] != sha256_file(SEED_PATH):
        raise RuntimeError("generation seeds differ")
    ...
    for concept in selected:
        ...
        write_json_atomic(OUTPUT_DIR / f"{concept}.metadata.json", {
            "schema_version": 1,
            "procedure": "day66-exact-precursor-positive-rewrites-v1",
