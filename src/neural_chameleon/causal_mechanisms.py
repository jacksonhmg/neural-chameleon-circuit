...
def vectorized_direct_path_monitors(
    target: RealizedForwardCapture,
    source: RealizedForwardCapture,
    component_groups: Sequence[Sequence[MechanismComponent]],
    layers: Sequence[nn.Module],
    *,
    monitor_layer: int = 12,
) -> Tensor:
    """Evaluate independent frozen-write groups with batched branch recomputation."""
    if not component_groups:
        raise ValueError("direct-path component groups must be nonempty")
    if not torch.equal(target.response_ids, source.response_ids) or not torch.equal(
        target.response_mask, source.response_mask
    ):
        raise ValueError("source and target direct-path captures do not align")
    normalized_groups: list[tuple[MechanismComponent, ...]] = []
    for group in component_groups:
        components = tuple(group)
        if not components or len({item.component_id for item in components}) != len(
            components
        ):
            raise ValueError("direct-path groups must be nonempty and unique")
        if any(component.layer > monitor_layer for component in components):
            raise ValueError("direct-path component occurs after the monitor")
        normalized_groups.append(components)

    group_count = len(normalized_groups)
    hidden = target.initial_residual.values.unsqueeze(0).repeat(group_count, 1, 1, 1)
    for layer_index in range(monitor_layer + 1):
        target_attention = target.attention_branches[layer_index].values
        attention_branches = target_attention.unsqueeze(0).repeat(
            group_count, 1, 1, 1
        )
        groups_with_heads = []
        heads_by_group = []
        for group_index, components in enumerate(normalized_groups):
            heads = tuple(
                int(component.head)
                for component in components
                if component.kind == "head" and component.layer == layer_index
            )
            if heads:
                groups_with_heads.append(group_index)
                heads_by_group.append(heads)
        if groups_with_heads:
            ...
            device = attention.o_proj.weight.device
            dtype = attention.o_proj.weight.dtype
            target_joint = target_raw.values.to(device=device, dtype=dtype).reshape(
                target_raw.values.shape[0],
                target_raw.values.shape[1],
                num_heads,
                head_dim,
            )
            source_joint = source_raw.values.to(device=device, dtype=dtype).reshape_as(
                target_joint
            )
            joint = target_joint.unsqueeze(0).repeat(
                len(groups_with_heads), 1, 1, 1, 1
            )
            mask = target_raw.response_mask.to(device).unsqueeze(-1)
            for local_index, heads in enumerate(heads_by_group):
                for head in heads:
                    if not 0 <= head < num_heads:
                        raise ValueError("counterfactual head is outside model geometry")
                    joint[local_index, :, :, head, :] = torch.where(
                        mask,
                        source_joint[:, :, head, :],
                        joint[local_index, :, :, head, :],
                    )
            batch, width = target_raw.values.shape[:2]
            target_expanded = target_joint.unsqueeze(0).repeat(
                len(groups_with_heads), 1, 1, 1, 1
            )
            with torch.inference_mode():
                target_recomputed = layer.post_attention_layernorm(
                    attention.o_proj(
                        target_expanded.reshape(
                            len(groups_with_heads) * batch, width, -1
                        )
                    )
                ).reshape(len(groups_with_heads), batch, width, -1)
                counterfactual_recomputed = layer.post_attention_layernorm(
                    attention.o_proj(
                        joint.reshape(len(groups_with_heads) * batch, width, -1)
                    )
                ).reshape(len(groups_with_heads), batch, width, -1)
                anchored = target_attention.to(device=device, dtype=torch.float32)[
                    None
                ] + (
                    counterfactual_recomputed.float() - target_recomputed.float()
                )
            attention_branches[groups_with_heads] = anchored.detach().cpu().to(
                target_attention.dtype
            )
        hidden = hidden + attention_branches.to(hidden.dtype)

        target_mlp = target.mlp_branches[layer_index].values
        mlp_branches = target_mlp.unsqueeze(0).repeat(group_count, 1, 1, 1)
        selected_mlp_groups = [
            group_index
            for group_index, components in enumerate(normalized_groups)
            if any(
                component.kind == "mlp" and component.layer == layer_index
                for component in components
            )
        ]
        if selected_mlp_groups:
            source_mlp = source.mlp_branches[layer_index].values
            mlp_branches[selected_mlp_groups] = source_mlp.unsqueeze(0).repeat(
                len(selected_mlp_groups), 1, 1, 1
            )
        hidden = hidden + mlp_branches.to(hidden.dtype)
    return hidden
